...
export default function NewLocationPage({ profile }) {
  ...
  function validate() {
    ...
    if (
      ...
      if (!e.latitude && !e.longitude) {
        e.longitude = 'Both latitude and longitude are required together (or leave both empty).';
      }
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      console.error('[NewLocation] Submit error:', err);
  ...
  return (
    <>
      <Head>
        <title>Add Location — Josephdeliverycompany Admin</title>
        <meta
          name="description"
          content="Add a new location to the Josephdeliverycompany network."
        />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <AdminLayout title="Add Location" profile={profile}>
        {result ? (
          <SuccessState result={result} onReset={resetForm} />
        ) : (
