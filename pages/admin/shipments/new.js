...
export default function NewShipmentPage({ profile }) {
  ...
  function validate() {
    ...
    for (const [key, label] of dateFields) {
      const raw = form[key];
      if (!raw) {
        if (key === 'shipmentDate' || key === 'eventDate') {
      ...
      if (!Number.isFinite(d.getTime())) {
        e[key] = `${label} must be a valid date.`;
      }
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      console.error('[NewShipment] Submit error:', err);
  ...
  return (
    <>
      <Head>
        <title>Create Shipment — Josephdeliverycompany Admin</title>
        <meta
          name="description"
          content="Create a new shipment in the Josephdeliverycompany admin portal."
        />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <AdminLayout title="Create Shipment" profile={profile}>
        {result ? (
          <SuccessState
            result={result}
            onReset={resetForm}
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <SectionCard
              ...
              title="Status & Initial Event"
              subtitle="Shipment status and first tracking event"
            >
              <Grid cols={3}>
                <Field
                  label="Status"
                ...
                <Field
                  ...
                  input={
                    <TextInput
                      ...
                      placeholder="e.g. Shipment booked and received at origin facility"
                    />
                  }
                />
                <Field
                  ...
                  input={
                    <TextInput
                      ...
                      onChange={(v) => updateField('eventLocation', v)}
...
/* ─── Success State (post-create) ────────────────────────────────────── */

function SuccessState({ result, onReset }) {
  const trackHref = `/track?tracking=${encodeURIComponent(result.trackingNumber)}`;
  const detailHref = `/admin/shipments/${result.id}`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Use the tracking number below to share with the customer.
        </p>
      </div>
