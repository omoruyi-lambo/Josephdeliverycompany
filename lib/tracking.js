 ...
 *
 * This module contains all tracking validation and demo logic.
 * It is intentionally isolated so the demo logic can be replaced
 * with a real database query (e.g. Supabase) later without
 * touching any UI components.
 */

/**
 * Accepted tracking number formats:
 *   - JDC-YYYY-NNNNN  (e.g. JDC-2026-00127)
...
export function validateTrackingInput(input) {
  ...
  if (!isValidTrackingNumber(normalised)) {
    return {
      valid: false,
      error: 'Please enter a valid tracking number (e.g. JDC-2026-00127).',
      trackingNumber: null,
    };
  }
